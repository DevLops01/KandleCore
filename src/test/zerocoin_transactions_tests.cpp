...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zkndlTracker = unique_ptr<CzKNDLTracker>(new CzKNDLTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZKNDL_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZKNDL_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
