...
CzKNDLWallet::CzKNDLWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zkndl seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZKNDLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZKNDLSeed_deprecated()) {
                    LogPrintf("%s: Updated zKNDL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zkndl seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zkndl wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzKNDLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZKNDLCount(nCountLastUsed);
    else if (!walletdb.ReadZKNDLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzKNDLWallet::Lock()
{
    seedMaster = 0;
}

void CzKNDLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzKNDLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZKNDL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzKNDLWallet::LoadMintPoolFromDB()
{
...
void CzKNDLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzKNDLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzKNDLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zkndlTracker->HasPubcoinHash(pMint.first)) {
...
bool CzKNDLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZKNDL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zkndlTracker which also adds to database
    pwalletMain->zkndlTracker->Add(dMint, true);
...
void CzKNDLWallet::SeedToZKNDL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzKNDLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzKNDLWallet::GenerateDeterministicZKNDL(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzKNDLWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzKNDLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
