// Copyright (c) 2019 The Kandle developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <primitives/deterministicmint.h>
#include "zkndltracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzKNDLTracker::CzKNDLTracker(std::string strWalletFile)
{
...
CzKNDLTracker::~CzKNDLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzKNDLTracker::Init()
{
...
bool CzKNDLTracker::Archive(CMintMeta& meta)
{
...
bool CzKNDLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzKNDLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzKNDLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzKNDLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzKNDLTracker::GetSerialHashes()
{
...
CAmount CzKNDLTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzKNDLTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzKNDLTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzKNDLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzKNDLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzKNDLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzKNDLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzKNDLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzKNDLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzKNDLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzKNDLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzKNDLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzKNDLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzKNDLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzKNDLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzKNDLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzKNDLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzkndl from DB\n", __func__, listDeterministicDB.size());
    }
