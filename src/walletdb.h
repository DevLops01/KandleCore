...
public:
    ...
    bool WriteZKNDLSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZKNDLSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZKNDLSeed_deprecated(uint256& seed);
    bool EraseZKNDLSeed();
    bool EraseZKNDLSeed_deprecated();

    bool WriteZKNDLCount(const uint32_t& nCount);
    bool ReadZKNDLCount(uint32_t& nCount);
