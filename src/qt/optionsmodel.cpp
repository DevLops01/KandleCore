...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::KNDL);
    ...
    if (!settings.contains("nAnonymizeKandleAmount"))
        settings.setValue("nAnonymizeKandleAmount", 1000);

    nAnonymizeKandleAmount = settings.value("nAnonymizeKandleAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeKandleAmount"))
        SoftSetArg("-anonymizekandleamount", settings.value("nAnonymizeKandleAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in kandle.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKandleAmount:
            return QVariant(nAnonymizeKandleAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeKandleAmount:
            nAnonymizeKandleAmount = value.toInt();
            settings.setValue("nAnonymizeKandleAmount", nAnonymizeKandleAmount);
            emit anonymizeKandleAmountChanged(nAnonymizeKandleAmount);
            break;
        case CoinControlFeatures:
