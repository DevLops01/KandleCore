...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered kandle address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 KNDL.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal kandle:URI (kandle:XyZ...?message=example)
            {
...
static void NotifyzKNDLReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzKNDLReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzKNDLReset.connect(boost::bind(NotifyzKNDLReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzKNDLReset.disconnect(boost::bind(NotifyzKNDLReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zkndlTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
